...
def lilyCantus(cantus):
    ...
    p = subprocess.Popen(
        ["lilypond", "--svg", "-s", "score.ly"], cwd="scores/" + cantus["folder"]
    )
    p.wait()
    # get rid of leaking garbage in the svg-file:
    p = subprocess.Popen(
        ["sed", "-E", "-i", 's/xlink:href="textedit:[^"]*"//g', "score.svg"],
    ...
    p = subprocess.Popen(
        [
            ...
            "-o",
            "out/" + cantus["file"] + ".svg",
            "scores/" + cantus["folder"] + "/score.svg",
        ],
    ...
    p.wait()
    copyfile(
        "scores/" + cantus["folder"] + "/score.midi", "out/" + cantus["file"] + ".midi"
    )
...
def renderBook(canti):
    ...
    with open("book/LiberCantorum.tex", "w") as f:
        f.write(texout)
    p = subprocess.Popen(
        ["pdflatex", "-interaction", "batchmode", "LiberCantorum.tex"],
        cwd="book/",
        stderr=subprocess.DEVNULL,
        stdout=subprocess.DEVNULL,
    )
